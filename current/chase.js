...
const Heat = {
  ...
  busted() {
    ...
    const mia = MIAMI() || DAYTONA() || KEYS() || ORLANDO(), cop = MIAMI() ? 'OFFICER' : DAYTONA() || KEYS() || ORLANDO() ? 'DEPUTY' : 'RHONDA', fine = Math.min(Game.money, 20), took = ['beer', 'joint', 'shroom', 'powder'].filter(k => Game.inv[k] > 0), bribe = mia ? 40 : 25;
    ...
    say([[cop, pick(['Gotcha. You run like a wet sock, Dan.', 'End of the line, Dan. Hands where I can see ’em. Not THERE.', 'Dan. You were jogging. I was DRIVING.'])],
      ...(hasUp('bail') && !Game.day_.bailUsed ? [['DAN', '', [['Flash Bubba’s Bail Card (1/day)', () => { Game.day_.bailUsed = true; Sound.play('cash'); return [['', 'Dan produces a laminated card. It says BUBBA’S BAIL BONDS · “HE’S WITH ME.”'], [cop, '...Bubba vouches for you? Bubba. The man with the boat named BAIL. Fine. GO.']]; }],
        ...(Game.money >= bribe ? [[`Slip ${mia ? 'him' : 'her'} $${bribe}`, payOff]] : []), ['Take the fine', takeFine]]]]
      : Game.money >= bribe ? [['DAN', '', [[`Slip ${mia ? 'him' : 'her'} $${bribe} (no fine, no headline)`, payOff], ['Take the fine', takeFine]]]] : takeFine())]);
  },
  escaped(how) {
    this.end();
    const H = {
